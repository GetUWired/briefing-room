...
public function __construct( $i2sdk ) {
$this->i2sdk = $i2sdk;
...
$pass_error = ! empty( $_GET['is_auth_passkey'] ) ? $this->manage_oauth_passkey( 'validate',
 $_GET['is_auth_passkey'] ) : false;
if( $pass_error ){
$this->manage_cron_hourly_refresh_check( 'disconnect' );
$this->i2sdk->admin_notice( 'connection',
 '', $pass_error );
return;
}
...
if( $new_token ) {
$this->i2sdk->setConfigurationOption( 'oauth_enabled',
 1 );
...
if ( empty( $_POST['is_set_oauth_expiration'] ) || $_POST['is_set_oauth_expiration'] <> 'Force Token Renewal' ) {
return;
}
...
$token = get_option( self::TOKEN_SLUG, false );

return $token;
}

public function delete_token_object( $log = false ){
update_option( self::REFRESH,
 0 );
...
$this->i2sdk->write_api_log( $log );

}
}

public function disconnect( $admin = 0 ){
$log = false;
if ( (int) $admin > 0 ) {
global $wpdb;
...
private function set_token_object( $data, $action ) {
$token = [
'access_token' => '',
...
if ( is_array( $data ) ) {
$scope_parts = array_key_exists( 'scope',
 $data ) ? array_filter( explode( "|", $data['scope'] ) ) : false;
if ( $scope_parts ) {
...
$verified = $this->verify_api_connection( $access_token,
 $appname );
$access_token = $verified ? $access_token : '';
$this->token_object = $verified ? $this->token_object : null;
update_option( self::TOKEN_SLUG,
 $this->token_object );
$this->i2sdk->setConfigurationOption( 'access_token', $access_token );
if ( $verified ) {
$this->i2sdk->setApp( $appname,
 $this->token_object, $action );
...
function verify_api_connection( $token, $appname ){
$this->i2sdk->m4is_dzjdf8h0p8gzs37xpyhh4krbgavss4()->m4is_z4cxfpr19pfbbv3rxv8hpqx( $token );

$this->i2sdk->m4is_dzjdf8h0p8gzs37xpyhh4krbgavss4()->m4is_8mfe8rwbqyk7d0gnn0fp5epafy20( $appname );
return $this->i2sdk->m4is_dzjdf8h0p8gzs37xpyhh4krbgavss4()->verify_api_connection();
...
private function is_token_expiring_soon() : bool {
$token = $this->get_token_object();
if( ! is_object( $token ) ) {
return false;

}
$timestamp = (int) get_option( self::REFRESH, 0 );
$horizon = time() + ( 8 * HOUR_IN_SECONDS );
$can_process = ( time() - MINUTE_IN_SECONDS ) > $timestamp;

$expired = $horizon > (int) $token->endOfLife;
...
if ( $action === 'connecting' ) {
wp_schedule_event( time() + mt_rand( 300,
900 ), 'hourly', $cron_hook );
}
}
...
$verified = $this->verify_api_connection( $access_token, $appname );
...
$this->log['caller'] = m4is_mh9t97q4p4p87vkr5zxs6k99b::m4is_smqht7cx0cpbf1ps13w89x8c69z9n( 'ISO-8859-1',
 'UTF-8', var_export( $body, true ) ); 
if ( is_wp_error( $response ) ) {
if( $this->api_log ){
$this->log['result'] = "Error Code : " . $response->get_error_code() . " | Message : " . $response->get_error_message();

$this->i2sdk->write_api_log($this->log);
}
}
else{
...
if( $this->api_log ){
$this->log['result'] = m4is_mh9t97q4p4p87vkr5zxs6k99b::m4is_smqht7cx0cpbf1ps13w89x8c69z9n( 'ISO-8859-1',
 'UTF-8', var_export( $token, true ) ); 
$this->i2sdk->write_api_log($this->log);
}
return $token;
}
...
function handle_http_invalid_access_token( $response,
 $args, $url ){
...
$url = str_replace("access_token={$expired}",
 "access_token={$token}", $url);
...
function manage_oauth_passkey( $action,
 $check_pass = false ) {
$transient_slug = 'memberium/keap/oauth/passkey';
if( $action === 'set' ){
$passkey = $this->get_passkey();

set_transient( $transient_slug, $passkey, 300 );
return $passkey;
}
...
$check = $check ? str_replace('_[memb_site_url]_',
 '', $check) : $check;
$siteurl = $this->get_siteurl();
if( $siteurl != $check ){
$this->log['result'] = sprintf( __("Site URL (%s) does not match Token URL (%s)" ),
 $siteurl, $check );
$this->delete_token_object( $this->log );
...
return substr_replace( $url,
 '_[memb_site_url]_', strlen( $url ) / 2, 0 );
}
