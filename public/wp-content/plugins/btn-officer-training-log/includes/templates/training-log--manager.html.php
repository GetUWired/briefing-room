...
(function ($) {
    ...
    officerList.on('change', 'input[type="checkbox"]', () => {
        const search = officerList.find('input[type="search"]');
        if (search.val()) {
    ...
    /* -------------------------------------------------------------
     *  OFFICER SEARCH (RIGHT SIDE)
     * ------------------------------------------------------------- */
    const officerSearch = $('#training-log-officer-search');
    let debounce;

    const searchInput = officerSearch.find('input[type="search"]').first()[0];
    ...
    /* -------------------------------------------------------------
     *  ADD STUDENT FROM SEARCH RESULTS
     * ------------------------------------------------------------- */
    officerSearch.on('click', '[data-student]', (e) => {
        const dataElm = e.currentTarget;
        ...
        officerList.find('ul').append(`
            <li>
                <label>
                    <input type="checkbox" name="students[]" value="${userId}" checked>
    ...
    /* -------------------------------------------------------------
     *  CLEAR VALIDATION WHEN CHECKBOXES CHANGE
     * ------------------------------------------------------------- */
    $(document).on('change', 'input[name="students[]"]', () => {
        searchInput.setCustomValidity('');
    });
     ...
     * ------------------------------------------------------------- */
    $('#training-log-form').on('submit', function (e) {

        // Always clear previous validity
        searchInput.setCustomValidity('');

        const hasSelected = $('input[name="students[]"]:checked').length > 0;

        if (!hasSelected) {
            e.preventDefault();
            searchInput.setCustomValidity('Please select at least one student for this training');
            searchInput.reportValidity();
        }
    });
...
/* -------------------------------------------------------------
 *  ADD NEW OFFICER (AJAX)
 * ------------------------------------------------------------- */
jQuery(function ($) {
    $('#add-new-officer').submit((e) => {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: (res) => {
                if (res.success) {
                    $('#add-new-officer').trigger('reset');

                    $('.aw-alert').addClass('bg-success').html(res.data.message).show();

                    $('#training-log-officer-list ul').append(`
                        <li>
                            <label>
                                <input type="checkbox" name="students[]" value="${res.data.id}" checked>
                    ...
                    $('.aw-alert').removeClass('bg-success').html(res.data[0].message).show();
                }
            }
        });
    });
