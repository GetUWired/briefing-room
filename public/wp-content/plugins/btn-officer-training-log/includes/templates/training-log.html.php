...
$agencyId = \BTN\BriefingRoom\Helpers\Memberium::getContactField('_AgencyID') ?: 1;

if(!$agencyId) {
    echo "Agency ID not found. Check the current user's contact record.";
    return;
}
...
<dialog id="add-officer-dialog">
    ...
    >
        ...
        <div>
            ...
            <input type="text" name="agencyId" id="agency" value="<?php echo $agencyId; ?>" readonly />
        </div>
    ...
    (($) => {
        ...
        officerSearch.find('input[type="search"]').on('input', (e) => {
            ...
            debounce = setTimeout(function(){
                ...
                $.when(studentsRequest, managersRequest).done(function (studentsRes, managersRes) {
                    const students = studentsRes[0] || [];
                    const managers = managersRes[0] || [];

                    officerSearch.find('ul').empty();

                    if (0 === students.length && 0 === managers.length) {
        ...
        $(document).on('change', 'input[name="students[]"], input[name="officers[]"]', () => {
            searchInput.setCustomValidity('');
        });
        ...
        $('#training-log-form').on('submit', function (e) {
            ...
            const hasSelected = $('input[name="students[]"]:checked, input[name="officers[]"]:checked').length > 0;
